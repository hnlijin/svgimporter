...
//                        Debug.Log("Mesh has changed: "+name);
        ...
        public static bool cloneSVGAsset(SVGAsset asset, out SVGAsset newAsset) {
            newAsset = ScriptableObject.CreateInstance<SVGAsset>();
            newAsset._antialiasing = asset._antialiasing;
            newAsset._border = asset._border;
            newAsset._compressDepth = asset._compressDepth;
            newAsset._depthOffset = asset._depthOffset;
            newAsset._ignoreSVGCanvas = asset._ignoreSVGCanvas;
            newAsset._meshCompression = asset._meshCompression;
            newAsset._scale = asset._scale;
            newAsset._format = asset._format;
            newAsset._useGradients = asset.useGradients;
            newAsset._pivotPoint = asset.pivotPoint;
            newAsset._vpm = asset.vpm;
            newAsset._sharedGradients = asset._sharedGradients;
            newAsset._generateCollider = asset._generateCollider;
            newAsset._generateNormals = asset._generateNormals;
            newAsset._generateTangents = asset._generateTangents;
            newAsset._sliceMesh = asset._sliceMesh;
            newAsset._optimizeMesh = asset._optimizeMesh;
            newAsset._keepSVGFile = asset._keepSVGFile;
            newAsset._sharedShaders = asset._sharedShaders;
            newAsset._canvasRectangle = asset._canvasRectangle;
            return true;
        }
        ...
        /// </summary>
        public static IEnumerator Load(string svgText, List<SVGAsset> svgAssets, SVGImporterSettings settings = null, int sliceLayerNum = 100)
        {
            if(string.IsNullOrEmpty(svgText)) yield break;
            if(settings == null)
            {
            ...
            SVGDocument svgDocument = new SVGDocument(svgText, graphics);
            SVGElement rootSVGElement = svgDocument.rootElement;
            ...
            // Create actual Mesh
            Shader[] outputShaders = null;
            SVGLayer[] layers = SVGGraphics.layers.ToArray();
            Mesh[] meshs = null;
            IEnumerator enumerator = SVGMesh.CombineMeshes(layers, (ms) => meshs = ms, (osh) => outputShaders = osh, asset._useGradients, asset._format, asset._compressDepth, asset._antialiasing, sliceLayerNum);
            yield return enumerator;
            if(meshs.Length <= 0) yield break;
            Mesh mesh = meshs[0];
            asset.layers = layers;
            ...
            if (meshs != null) {
                for (int i = 1; i < meshs.Length; i++) {
                    SVGAsset newAsset = null;
                    if (cloneSVGAsset(asset, out newAsset)) {
                        svgAssets.Add(newAsset);
                        if(!newAsset.useLayers) {
                            newAsset._sharedMesh = meshs[i];
                        }
                        yield return new WaitForEndOfFrame();
                    }
                }
